...
export const PointAllocator = () => {
    ...
    const handleSubtotalChange = (id: number, subtotalIdStr: string) => {
        const subtotalId = parseInt(subtotalIdStr, 10);
        const newSubtotalIds = isNaN(subtotalId) ? [] : [subtotalId];
        handlePointPropChange(id, 'subtotalIds', newSubtotalIds);
    };
    ...
    const applyBulkSubtotal = () => {
        const subId = parseInt(bulkSubtotal, 10);
        const newIds = isNaN(subId) ? [] : [subId];
    ...
    return (
        <div className="w-full h-full flex flex-col relative overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto bg-slate-100 dark:bg-slate-900/50 p-2 rounded-md pb-32">
                <div className={`grid gap-2 ${showImages ? 'grid-cols-1' : 'grid-cols-1'}`}>
                {internalPoints.map((point) => {
                    ...
                    const isMarkSheet = area.type === AreaType.MARK_SHEET;

                    return (
                        <div 
                            key={point.id} 
                            className={`rounded-lg shadow-sm border transition-all cursor-pointer ${isSelected ? 'bg-sky-50 dark:bg-sky-900/30 border-sky-400 ring-2 ring-sky-400/20' : 'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700'} ${showImages ? 'p-4' : 'p-2 flex items-center gap-4'}`}
                            onClick={() => toggleSelect(point.id)}
                        >
                            {/* Header Section: Checkbox & Name */}
                            <div className={`flex items-center gap-3 ${showImages ? 'mb-2' : 'w-1/4 min-w-[200px]'}`}>
                                ...
                                <div className="min-w-0 flex-1">
                                    <div className="font-bold text-slate-700 dark:text-slate-300 truncate text-sm" title={point.label}>{point.label}</div>
                                    <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold uppercase tracking-wider ${isMarkSheet ? 'bg-teal-100 text-teal-700 dark:bg-teal-900 dark:text-teal-300' : 'bg-blue-100 text-blue-700 dark:bg-blue-900 dark:text-blue-300'}`}>
                                        {isMarkSheet ? 'マーク' : '記述'}
                                    </span>
                                </div>
                            </div>

                            {/* Image Section (Only if showImages is true) */}
                            {showImages && imageSrc && (
                            ...
                            {/* Controls Section */}
                            <div className={`grid gap-4 ${showImages ? 'grid-cols-1 md:grid-cols-3' : 'flex-1 grid-cols-4 flex items-center'}`} onClick={e => e.stopPropagation()}>
                                {/* Points Input */}
                                <div className={`space-y-1 ${showImages ? '' : 'flex items-center gap-2 space-y-0 w-24'}`}>
                                    <label className="text-xs font-bold text-slate-500 whitespace-nowrap">配点</label>
                                    <input 
                                        type="number" min="0" 
                                        value={point.points} 
                                        onChange={(e) => handlePointPropChange(point.id, 'points', parseInt(e.target.value) || 0)} 
                                        className="w-full bg-white dark:bg-slate-700 border border-slate-300 dark:border-slate-600 rounded p-1.5 text-sm" 
                                    />
                                </div>

                                {/* Subtotal Selector */}
                                <div className={`space-y-1 ${showImages ? '' : 'flex items-center gap-2 space-y-0 flex-1'}`}>
                                    <label className="text-xs font-bold text-slate-500 whitespace-nowrap">小計分類</label>
                                    <select
                                        value={point.subtotalIds?.[0] || ''}
                                        onChange={(e) => handleSubtotalChange(point.id, e.target.value)}
                                        className="w-full bg-white dark:bg-slate-700 border border-slate-300 dark:border-slate-600 rounded p-1.5 text-xs"
                                    >
                                        <option value="">(なし)</option>
                                        {subtotalAreas.map(sub => (
                                ...
                                {isMarkSheet ? (
                                    <>
                                        <div className={`space-y-1 ${showImages ? '' : 'flex items-center gap-2 space-y-0 w-24'}`}>
                                            <label className="text-xs font-bold text-slate-500 whitespace-nowrap">肢数</label>
                                            <input 
                                                type="number" min="2" max="10" 
                                                value={point.markSheetOptions || 4} 
                                                onChange={e => handlePointPropChange(point.id, 'markSheetOptions', parseInt(e.target.value) || 2)} 
                                                className="w-full bg-white dark:bg-slate-700 border border-slate-300 dark:border-slate-600 rounded p-1.5 text-sm" 
                                            />
                                        </div>
                                        <div className={`space-y-1 ${showImages ? '' : 'flex items-center gap-2 space-y-0 w-32'}`}>
                                            ...
                                            <input 
                                                ...
                                                className="w-full bg-white dark:bg-slate-700 border border-slate-300 dark:border-slate-600 rounded p-1.5 text-sm font-bold text-green-600 dark:text-green-400" 
                                            />
                                        </div>
                                    </>
                                ) : (
                                    // Spacers for alignment in list view
                                    !showImages && (
                                        <>
            ...
            {/* Bulk Action Bar */}
            {selectedIds.size > 0 && (
                <div className="absolute bottom-24 left-1/2 -translate-x-1/2 bg-slate-800 text-white p-3 rounded-2xl shadow-2xl flex items-center gap-4 z-50 border border-slate-600 animate-in slide-in-from-bottom-4 overflow-x-auto max-w-[95%]">
                    <div className="flex items-center gap-2 px-2 py-1 bg-slate-700 rounded-lg whitespace-nowrap">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                        ...
                        <input type="number" placeholder="-" className="w-14 p-1 text-black rounded text-sm font-bold" value={bulkPoints} onChange={e => setBulkPoints(e.target.value)} />
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                        <span className="text-xs font-bold text-slate-300">小計:</span>
                        <select
                            value={bulkSubtotal}
                            onChange={(e) => setBulkSubtotal(e.target.value)}
                            className="w-32 p-1 text-black rounded text-xs"
                        >
                        ...
                        <button onClick={applyBulkSubtotal} className="px-3 py-1 bg-sky-600 hover:bg-sky-500 rounded-md text-xs font-bold transition-colors">適用</button>
                    </div>
