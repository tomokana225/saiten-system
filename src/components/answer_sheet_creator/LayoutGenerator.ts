            ...
            section.questions.forEach((q, idx) => {
                ...
                if (q.type === 'marksheet') {
                    ...
                    placeCell(currentRow, absCol + qNumBoxWidth, answerBoxWidth, createCell({
                        ...
                        rowSpan: englishRows,
                        type: 'english-grid',
                        metadata: { wordCount, wordsPerLine, lineHeightRatio }, 
                        borders: { top: true, bottom: true, left: true, right: true }
                    }));

                    // ★ Calculate height for each english row
                    const perRowHeightPx = baseRowHeightMm * lineHeightRatio * mmToPx;

                    for (let r = 0; r < englishRows; r++) {
                        const rIdx = currentRow + r;
                        if (rIdx >= rowHeights.length) addRow();
                        rowHeights[rIdx] = Math.max(rowHeights[rIdx] || 0, perRowHeightPx);
                    }
                } else {
