...
export const Print: React.FC<PrintProps> = ({ initialTab, questionStats, onClose }) => {
    ...
    const renderSettings = () => {
        ...
        if (activeTab === 'sheets') {
             return (<div className="space-y-4">
                <div className="p-2 rounded-lg bg-slate-200 dark:bg-slate-800/50"><label className="font-medium text-sm text-slate-700 dark:text-slate-300">並び順</label><div className="flex items-center gap-2 mt-1"><button key="rank" onClick={() => setSortOrder('rank')} className={`px-3 py-1 text-xs rounded-md flex-1 ${sortOrder === 'rank' ? 'bg-sky-500 text-white' : 'bg-slate-50 dark:bg-slate-700'}`}>点数順</button><button key="number" onClick={() => setSortOrder('number')} className={`px-3 py-1 text-xs rounded-md flex-1 ${sortOrder === 'number' ? 'bg-sky-500 text-white' : 'bg-slate-50 dark:bg-slate-700'}`}>番号順</button></div></div>
                <div className="p-2 rounded-lg bg-slate-200 dark:bg-slate-800/50"><label className="font-medium text-sm text-slate-700 dark:text-slate-300">採点マーク (◯☓△)</label><div className="flex items-center gap-4 mt-2"><label className="flex items-center text-xs gap-1"><input type="checkbox" checked={layoutSettings.mark.show} onChange={e => handleSettingChange('mark', 'show', e.target.checked)} /> 表示</label></div><div className="mt-2 space-y-2"><label className="text-xs font-medium text-slate-600 dark:text-slate-400">配置基準</label><div className="flex items-center gap-2"><button onClick={() => handleSettingChange('mark', 'positioningMode', 'answer_area')} className={`px-3 py-1 text-xs rounded-md flex-1 ${layoutSettings.mark.positioningMode === 'answer_area' ? 'bg-sky-500 text-white' : 'bg-slate-50 dark:bg-slate-700'}`}>解答欄</button><button onClick={() => handleSettingChange('mark', 'positioningMode', 'question_number_area')} disabled={!hasLinkedQuestionNumbers} className={`px-3 py-1 text-xs rounded-md flex-1 ${layoutSettings.mark.positioningMode === 'question_number_area' ? 'bg-sky-500 text-white' : 'bg-slate-50 dark:bg-slate-700'} disabled:opacity-50 disabled:cursor-not-allowed`}>問題番号</button></div>{!hasLinkedQuestionNumbers && <p className="text-xs text-slate-500">問題番号に配置するには、配点設定画面でエリアを紐付ける必要があります。</p>}</div><div className="flex items-center gap-2 mt-1"><span>サイズ:</span><input type="number" value={layoutSettings.mark.fontSize} onChange={e => handleSettingChange('mark', 'fontSize', parseInt(e.target.value))} className="w-16 p-1 text-xs rounded-md bg-white dark:bg-slate-700" /></div><div className="flex items-center gap-2 mt-1"><span>不透明度:</span><input type="range" min="0" max="1" step="0.1" value={layoutSettings.mark.opacity} onChange={e => handleSettingChange('mark', 'opacity', parseFloat(e.target.value))} className="w-full" /></div><div className="grid grid-cols-3 gap-2 mt-2"><div className="text-center"><label className="text-xs">◯の色</label><input type="color" value={layoutSettings.mark.correctColor} onChange={e => handleSettingChange('mark', 'correctColor', e.target.value)} className="w-full h-8 p-0 border-none rounded cursor-pointer bg-transparent" /></div><div className="text-center"><label className="text-xs">☓の色</label><input type="color" value={layoutSettings.mark.incorrectColor} onChange={e => handleSettingChange('mark', 'incorrectColor', e.target.value)} className="w-full h-8 p-0 border-none rounded cursor-pointer bg-transparent" /></div><div className="text-center"><label className="text-xs">△の色</label><input type="color" value={layoutSettings.mark.partialColor} onChange={e => handleSettingChange('mark', 'partialColor', e.target.value)} className="w-full h-8 p-0 border-none rounded cursor-pointer bg-transparent" /></div></div><AlignmentPicker hAlign={layoutSettings.mark.hAlign} vAlign={layoutSettings.mark.vAlign} onAlignChange={(prop, val) => handleSettingChange('mark', prop, val)} /><div className="flex items-center gap-2 mt-1"><span className="text-xs">横位置:</span><input type="range" min="-100" max="100" value={layoutSettings.mark.hOffset} onChange={e => handleSettingChange('mark', 'hOffset', parseFloat(e.target.value))} className="w-full" /><span className="text-xs w-10 text-right">{layoutSettings.mark.hOffset}%</span></div><div className="flex items-center gap-2 mt-1"><span className="text-xs">縦位置:</span><input type="range" min="-100" max="100" value={layoutSettings.mark.vOffset} onChange={e => handleSettingChange('mark', 'vOffset', parseFloat(e.target.value))} className="w-full" /><span className="text-xs w-10 text-right">{layoutSettings.mark.vOffset}%</span></div></div>
